...
TpmLiveTest::TpmLiveTest()
    : tpm_(Tpm::GetSingleton()), cryptohome_keys_manager_(tpm_, &platform_) {}
...
bool TpmLiveTest::TpmEccAuthBlockTest() {
  ...
  // device.
  if (!cryptohome_keys_manager_.HasCryptohomeKey(CryptohomeKeyType::kECC)) {
    LOG(INFO) << "Skipping the test: ECC is not supported by the TPM.";
    return true;
  }

  TpmEccAuthBlock auth_block(tpm_, &cryptohome_keys_manager_);
...
bool TpmLiveTest::TpmBoundToPcrAuthBlockTest() {
  ...
  TpmBoundToPcrAuthBlock auth_block(tpm_, &cryptohome_keys_manager_);
  if (!TestPasswordBasedAuthBlock(auth_block)) {
...
bool TpmLiveTest::TpmNotBoundToPcrAuthBlockTest() {
  ...
  TpmNotBoundToPcrAuthBlock auth_block(tpm_, &cryptohome_keys_manager_);
  if (!TestPasswordBasedAuthBlock(auth_block)) {
