...
bool Configuration::AddSysfsTrigger(int sysfs_trigger_id) {
  ...
  auto triggers = sensor_->GetContext()->GetTriggersByName(trigger_name);
  ...
  if (triggers.size() == 0) {
    ...
    auto iio_sysfs_trigger = sensor_->GetContext()->GetTriggerById(kSysfsTriggerId);
    ...
    triggers = sensor_->GetContext()->GetTriggersByName(trigger_name);
    if (triggers.size() != 1) {
